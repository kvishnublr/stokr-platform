...
# Upload scripts as postgres user so it runs with peer auth
sftp=s.open_sftp()
sftp.put(r"C:\Users\itsvi\Desktop\work_new\stokr-platform\bt.py", "/tmp/bt.py")
sftp.close()

# Run as postgres user for Unix socket peer auth
import select,time,sys
i,o,e=s.exec_command("su - postgres -c 'python3 /tmp/bt.py' 2>&1", get_pty=True)
