...
remote("""PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -c "SELECT table_name, column_name, numeric_precision, numeric_scale FROM information_schema.columns WHERE numeric_precision=12 AND numeric_scale=4 ORDER BY table_name;" """)
