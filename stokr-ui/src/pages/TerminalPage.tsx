    ...
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-3 py-1 text-[11px] font-bold uppercase tracking-widest text-violet-200">
              <Layers className="h-3.5 w-3.5" />
              Institutional terminal
            </div>
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">Execution workstation</h1>
          ...
          <GlassPanel className="px-5 py-3 text-right">
            <div className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Desk latency cue</div>
            <div className="font-mono text-xl text-neutral-100">≈ {latencyCue} ms</div>
            <div className="mt-2 text-[10px] text-neutral-500">Synthetic jitter from queued client events · wire HRC when ready.</div>
          </GlassPanel>
        </div>
      </motion.div>

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Order surface" sublabel="Idempotent intents" value="OMS" highlight />
        <MetricCard label="Realtime rail" trend="flat" sublabel="/ws multiplex" value={<span className="text-neutral-300">Healthy</span>} />
        <MetricCard label="Execution mode" trend="flat" sublabel="SIM default" value={<span>Paper</span>} />
        <MetricCard label="Operational feed" trend="up" value={feed.length.toString()} sublabel="Last 50 hooks" />
      </div>
      ...
      <GlassPanel className="p-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Latency legend</span>
        </div>
        <div className="mt-4 grid gap-4 sm:grid-cols-3 text-xs text-neutral-500">
          <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-3 text-emerald-100">
            <div className="font-semibold uppercase tracking-wide">Green</div>
            Sub-millisecond websocket fan-out from bridge + ack from OMS.
          </div>
          <div className="rounded-xl border border-amber-900/50 bg-amber-950/20 p-3 text-amber-100">
          ...
          <div className="rounded-xl border border-rose-900/55 bg-rose-950/20 p-3 text-rose-100">
