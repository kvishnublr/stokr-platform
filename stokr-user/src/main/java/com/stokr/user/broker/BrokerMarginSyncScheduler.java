 ...
 *
 * Each account is processed in its own auto-committed save (no method-level @Transactional): broker_accounts
 * is written by several schedulers (token refresh, status sync) and a single big transaction meant one
 * concurrent-update conflict rolled back the whole batch. Per-account saves are independent and idempotent,
 * so a conflict on one account is logged and retried next cycle without affecting the others.
 */
@Component
...
public class BrokerMarginSyncScheduler {
    ...
    public void sync() {
        ...
        for (BrokerAccount a : accounts) {
            try {
                saveAccountSnapshot(a);
            } catch (OptimisticLockingFailureException ex) {
                // Another broker_accounts writer committed first; idempotent, reconciles next cycle.
                log.debug("broker.sync.deferred id={} reason=concurrent_update", a.getId());
            } catch (Exception e) {
    ...
    private void saveAccountSnapshot(BrokerAccount a) throws Exception {
