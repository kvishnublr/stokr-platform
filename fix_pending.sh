...
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -c "UPDATE orders SET status='REJECTED', error_message='No broker account at time of order - intraday path fired before fix', updated_at=NOW() WHERE status='PENDING' AND broker_order_id IS NULL;"
echo "Done."

echo ""
echo "=== Verifying ==="
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -c "SELECT id, symbol, side, status, error_message, created_at::text FROM orders WHERE status='PENDING';"
