...
PGPASSWORD=stokr2026 psql -h localhost -U postgres -d stokr_lite -c "SELECT id, symbol, side, status, created_at::text FROM strategy_signals WHERE status IN ('GENERATED','EXECUTED') ORDER BY created_at DESC;"

echo ""
echo "=== ALL TABLES LIST ==="
PGPASSWORD=stokr2026 psql -h localhost -U postgres -d stokr_lite -c "SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_type='BASE TABLE' ORDER BY table_name;"
