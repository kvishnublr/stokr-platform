#!/bin/bash
docker exec -i stokr-postgres psql -U postgres stokr_platform <<'SQL'
UPDATE oms_orders o
SET state = 'CANCELLED',
...
SELECT count(*) as remaining_open FROM oms_orders WHERE state='FILLED' AND execution_mode='LIVE' AND signal_id IS NULL;
SQL
