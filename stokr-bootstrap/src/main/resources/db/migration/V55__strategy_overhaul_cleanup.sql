...
-- 1. Delete strategy_execution_configs referencing old definitions (FK constraint)
DELETE FROM strategy_execution_configs
WHERE strategy_id IN (
    SELECT id FROM strategy_definitions
    WHERE strategy_key NOT IN ('NSE_SPIKE_DETECTION')
);

-- 2. Delete strategy_instances referencing old definitions (FK constraint)
...
-- 3. Delete ALL runtime bindings for strategies being removed
...
-- 4. Hard-delete all old strategy definitions
DELETE FROM strategy_definitions
WHERE strategy_key NOT IN ('NSE_SPIKE_DETECTION');

-- 5. Insert 4 new strategy definitions (using actual column names from schema)
INSERT INTO strategy_definitions (id, strategy_key, display_name, description, asset_class, segment, default_exchange, default_timeframe, enabled, deleted, version, created_at, updated_at)
VALUES
...
-- 6. Create runtime bindings for new strategies with NIFTY_50 and NIFTY_100 universe groups
...
-- 7. Also create binding for NSE_SPIKE_DETECTION to NIFTY_50/NIFTY_100 if not existing
INSERT INTO strategy_runtime_bindings (id, strategy_catalog_id, universe_group_id, runtime_enabled, scan_interval_seconds, max_positions, risk_profile, created_at, updated_at)
