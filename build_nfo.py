...
i, o, e = s.exec_command(
    "cd /root/stokr-platform/stokr-lite && mvn package -pl backend -DskipTests -q 2>&1",
    get_pty=True)
...
success = exit_code == 0 and 'BUILD SUCCESS' in buf

if not success:
    # Collect errors
    print(f"\nBUILD FAILED (exit={exit_code}). Checking errors...")
    i2, o2, e2 = s.exec_command(
        "cd /root/stokr-platform/stokr-lite && mvn compile -pl backend 2>&1 | grep -E 'ERROR|error:' | tail -20",
        get_pty=True)
    time.sleep(3)
    while o2.channel.recv_ready():
        print(o2.channel.recv(4096).decode('utf-8', errors='replace'))
else:
    print("\nBUILD SUCCESS — rebuilding Docker...")
