  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-8 animate-fade-in-up">
        <div>
          <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Universe Groups</h1>
          <p className="text-slate-500 text-sm mt-2">Manage symbol universes and index constituents</p>
        </div>
      ...
      {showCreate && (
        <div className="card-light-strong rounded-2xl p-6 mb-6 animate-scale-in border-indigo-200/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Group Key</label>
            ...
            <div>
              ...
              <select value={form.universeType} onChange={e => setForm({...form, universeType: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-700 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all input-light">
...
function UniverseGroupCard({ group, expanded, onToggleExpand, onToggleEnabled, onDelete, onSync, showBulk, onToggleBulk, bulkSymbols, onBulkSymbolsChange, onBulkImport, onAddSymbol, onRemoveSymbol, availableKeys, delay }) {
  ...
  return (
    <div className={`card-light overflow-hidden animate-fade-in-up ${delay} hover-glow transition-all duration-300 ${expanded ? 'border-indigo-200/60 shadow-md' : ''}`}>
      {/* Header Row */}
      <div className="p-4 flex items-center justify-between cursor-pointer hover:bg-slate-50/80 transition-colors" onClick={onToggleExpand}>
        <div className="flex items-center gap-4">
          <div className={`px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
            group.autoManaged
              ? 'bg-emerald-50 text-emerald-700 border border-emerald-200 badge-glow'
              : 'bg-slate-100 text-slate-600 border border-slate-200'
          }`}>
            {group.autoManaged ? 'Auto' : 'Custom'}
          </div>
          <div>
            <p className="font-semibold text-sm text-slate-700">{group.displayName}</p>
            <p className="text-xs text-slate-400 mt-0.5">{group.groupKey} &middot; {group.assetClass} &middot; {group.exchange}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {group.autoManaged && availableKeys?.includes(group.groupKey) && (
            <button onClick={(e) => { e.stopPropagation(); onSync(); }} className="text-xs bg-indigo-50 text-indigo-600 px-3 py-1.5 rounded-lg border border-indigo-200 hover:bg-indigo-100 transition-all hover:shadow-sm">
              Sync
            </button>
          )}
          <button onClick={(e) => { e.stopPropagation(); onToggleEnabled(!group.enabled); }} className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${group.enabled ? 'bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100' : 'bg-slate-100 text-slate-500 border-slate-200 hover:bg-slate-200'}`}>
      ...
      {/* Expanded Content */}
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-100 animate-expand-down">
          <div className="mt-4 flex items-center gap-3">
            <input placeholder="Add symbol..." value={newSymbol} onChange={e => setNewSymbol(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-4 py-2 text-sm text-slate-700 placeholder-slate-400 focus:outline-none focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 transition-all flex-1 input-light" onKeyDown={e => { if (e.key === 'Enter') { onAddSymbol(newSymbol); setNewSymbol(''); } }} />
