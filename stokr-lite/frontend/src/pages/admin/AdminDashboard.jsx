  ...
  return (
    <div>
      ...
      <div className="mb-8 animate-fade-in-up">
        ...
        <p className="text-slate-500 text-sm mt-2">Platform monitoring and controls</p>
      </div>

      {/* Kill Switch Banner */}
      {killSwitch?.active && (
        <div className="card-light-strong rounded-2xl p-5 mb-8 flex items-center gap-4 animate-fade-in-up border-rose-200 animate-border-glow">
          ...
          <div>
            <p className="text-rose-600 font-bold text-sm uppercase tracking-wide">Kill Switch Active</p>
            <p className="text-rose-500/70 text-xs mt-1">All trading halted by {killSwitch.activatedBy} — {killSwitch.reason}</p>
          </div>
        </div>
      )}
      ...
      <div className="animate-fade-in-up delay-500">
        <div className="flex items-center gap-3 mb-5">
          ...
          <h2 className="text-lg font-semibold text-slate-800">Configuration &amp; Mappings</h2>
        </div>
...
function StatCard({ title, value, icon, color, delay }) {
  const gradients = {
    indigo: 'from-indigo-500/10 to-violet-500/10 border-indigo-200/50',
    emerald: 'from-emerald-500/10 to-teal-500/10 border-emerald-200/50',
    sky: 'from-sky-500/10 to-cyan-500/10 border-sky-200/50',
    amber: 'from-amber-500/10 to-orange-500/10 border-amber-200/50',
  };
  const iconColors = {
  ...
  return (
    <div className={`card-light p-5 hover-lift hover-glow animate-fade-in-up ${delay} relative overflow-hidden group`}>
      <div className={`absolute inset-0 bg-gradient-to-br ${gradients[color]} opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-2xl`} />
      <div className="relative z-10">
        ...
        <p className="text-3xl font-bold text-slate-800 stat-number">
...
function QuickLink({ to, title, desc, icon, color, delay }) {
  ...
  return (
    <a href={to} className={`card-light p-5 hover-lift hover-glow animate-fade-in-up ${delay} group block`}>
