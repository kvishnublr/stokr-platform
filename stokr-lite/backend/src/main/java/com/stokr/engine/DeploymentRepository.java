...
public interface DeploymentRepository extends JpaRepository<Deployment, Long> {
    ...
    @Query("SELECT COUNT(d) FROM Deployment d WHERE d.strategyId = :strategyId AND d.status = 'ACTIVE' AND d.mode = 'LIVE'")
    long countByStrategyIdAndActiveAndLive(Long strategyId);
