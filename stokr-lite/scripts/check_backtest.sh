...
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -t -A -c "SELECT DISTINCT symbol FROM candle_data WHERE timeframe='daily' ORDER BY symbol LIMIT 15;"
echo ""
echo "=== Total daily candles per symbol ==="
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -t -A -c "SELECT symbol, COUNT(*) FROM candle_data WHERE timeframe='daily' GROUP BY symbol ORDER BY COUNT(*) DESC LIMIT 10;"
echo ""
echo "=== NIFTY_50 universe symbols in strategies table ==="
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -t -A -c "SELECT id, name, description FROM strategies WHERE id IN (15,16,17,21,23);"
echo ""
