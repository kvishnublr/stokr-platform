...
PGPASSWORD=stokr2026 psql -h localhost -U postgres -d stokr_lite -c "SELECT MIN(timestamp)::date as earliest, MAX(timestamp)::date as latest, COUNT(*) FROM candle_data WHERE timeframe='daily';"
