...
PGPASSWORD=stokr2026 psql -h localhost -U postgres -d stokr_lite -t -A -c "SELECT column_name, data_type FROM information_schema.columns WHERE table_name='broker_accounts' ORDER BY ordinal_position;"
echo ""
echo "=== broker_accounts data ==="
PGPASSWORD=stokr2026 psql -h localhost -U postgres -d stokr_lite -c "SELECT id, broker, access_token, api_key, expires_at, last_refresh FROM broker_accounts;"
echo ""
