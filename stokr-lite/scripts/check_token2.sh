...
PGPASSWORD=`$POSTGRES_PASSWORD psql -h localhost -U postgres -d stokr_lite -c "SELECT id, broker_name, client_id, status, access_token IS NOT NULL as has_token, token_expiry, auto_reconnect, last_auto_reconnect FROM broker_accounts;"
echo ""
